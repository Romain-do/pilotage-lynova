...
export function Tresorerie({ data, todayISO }: { data: Data; todayISO: string }) {
  ...
  // Évolution rémunération exercice vs N-1 (axe fiscal oct→sept, indépendant du sélecteur).
  const fyNow = fyOf(todayISO);
  const remuFyCur = useMemo(() => remuByFiscalMonth(data.outflows, fyNow), [data.outflows, fyNow]);
  const remuFyPrev = useMemo(() => remuByFiscalMonth(data.outflows, fyNow - 1), [data.outflows, fyNow]);
